...
const setLighting = (scene: THREE.Scene) => {
  ...
  function setPointLight(screenLight: THREE.Mesh | null) {
    if (screenLight && (screenLight.material as THREE.MeshStandardMaterial).opacity > 0.9) {
      pointLight.intensity = (screenLight.material as THREE.MeshStandardMaterial).emissiveIntensity * 20;
    } else {
